let obj=[{"id":1,"first_name":"Chadwick","last_name":"Ayre","email":"cayre0@cam.ac.uk","gender":"Male","img_src":"https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1","class":11,"marks":18,"passing":false,"city":"Moorreesburg"},
    ...
    {
        let tablerow = document.getElementsByClassName("stdbody")[0];
      obj.forEach(element => {
    ...
    function search()
    {
        let existingFemaleTable = document.querySelector(".stdtablef");
        if (existingFemaleTable) {
            existingFemaleTable.remove();
        }
    ...
    function sortpassing()
    {
        ...
        let passingStudents = obj.filter(element => element.passing === true);
        let tablerow = document.getElementsByClassName("stdbody")[0];
        tablerow.innerHTML = '';
        generatetable(passingStudents);
    }
    function sortgender()
    {
        let tablerow = document.getElementsByClassName("stdbody")[0];
        tablerow.innerHTML='';
        let males=obj.filter(element=>element.gender==="Male")
        generatetable(males);
        let existingFemaleTable = document.querySelector(".stdtablef");
    if (existingFemaleTable) {
        existingFemaleTable.remove();
        ...
        newtable.setAttribute("class","stdtablef");
        newtable.innerHTML=` <thead>
                ...
                <tbody class="female-stdbody">
                </tbody>
        `
        document.body.appendChild(newtable);
        let femaleTableBody = newtable.querySelector(".female-stdbody");
        let females=obj.filter(elements=>elements.gender==="Female");
     ...
     <td><div class="name"><img src=${element.img_src} class="img"><span class=nam>${element.first_name} ${element.last_name}</span></div></td>
            ...
            <td>${element.email}</td>
            `
            femaleTableBody.appendChild(rowele)
          });
